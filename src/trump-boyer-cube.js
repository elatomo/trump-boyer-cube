...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const DEFAULT_CONFIG = {
    ...
    /** Geometry settings */
    scale: 2.5, // Controls overall cube size (node spacing and wireframe)
  ...
  // Read configuration from window.MAGIC_CUBE_CONFIG if available
  const CONFIG = mergeConfigs(DEFAULT_CONFIG, window.MAGIC_CUBE_CONFIG || {});

  // Collection of UI elements and 3D objects
  const elements = {
    nodes: [],
    nodeLabels: [],
  ...
  // Maps for tracking relationships between numbers and positions
  const data = {
    numberToPosition: {},
    positionToNumber: {},
  };
    ...
    for (let z = 0; z < 5; z++) {
      for (let y = 0; y < 5; y++) {
        for (let x = 0; x < 5; x++) {
          ...
          const posY = -(y - 2) * CONFIG.scale; // Inverted so Y+ is up
  ...
  // Initialize the scene with nodes
  createNodes();
  ...
  controls.addEventListener("end", function () {
    // Resume auto-rotation after a brief delay if enabled
  ...
  function animate() {
    ...
    // Update node number labels if enabled
    if (CONFIG.showNodeNumbers) {
      updateNodeLabels();
    }
